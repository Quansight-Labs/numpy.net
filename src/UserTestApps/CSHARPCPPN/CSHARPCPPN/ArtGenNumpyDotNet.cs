...
namespace CSHARPCPPN
{
    public class ArtGenNumpyDotNet
    {
        ...
        public void InitialiseCPPN(int batchSize = 1, int netSize = 32, int hSize = 32, int width = 256, int height = 256, float scaling = 1.0f, bool RGB = false)
        {
            ...
            Img_Batch = np.random.randn(new int[] { batchSize, width, height, C_Dim });

            Hid_Vec = np.random.randn(new int[] { batchSize, hSize });

            X_Dat = np.random.randn(batchSize, width * height, 1);
            Y_Dat = np.random.randn(batchSize, width * height, 1);
            R_Dat = np.random.randn(batchSize, width * height, 1);
        }
        ...
        public ndarray BuildCPPN(int width, int height, ndarray x_dat, ndarray y_dat, ndarray r_dat, ndarray hid_vec)
        {
            ...
            Art_Net = FullyConnected(h_vec_unwrapped, NetSize) +
                ...
                FullyConnected(y_dat_unwrapped, NetSize, false) +
        ...
        public ndarray TanhSig(int numOfLayers = 3)
        {
        ...
        {
            var mat = np.random.randn(input.shape.iDims[1], out_dim).astype(np.Float32);

            var result = np.matmul(input, mat);

            if (with_bias)
            {
                var bias = np.random.randn(1, out_dim).astype(np.Float32);
        ...
        public ndarray Generate(int width = 256, int height = 256, float scaling = 20.0f, ndarray z = null)
        {
            ...
            if (vector == null)
            {
                var randomUniforms = NumSharp.np.random.uniform(-1, 1, new int[] { BatchSize, HSize }).ToArray<double>();

                vector = np.array(randomUniforms).reshape(BatchSize, HSize);
            }
        ...
        public double[] GenerateArt(int batch_size = 1, int net_size = 16, int h_size = 8, int width = 512, int height = 512, float scaling = 10.0f, bool RGB = true, int? seed = null)
        {
            ...
            if (seed != null)
            {
                ...
                NumSharp.np.random.seed(KeyId.GetValueOrDefault());
            }
            else
            {
